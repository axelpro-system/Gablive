/**
 * Mirrors src/lib/emailTemplates.js for Edge Functions (keep in sync).
 */
    ...
    opts.ctaLabel && opts.ctaUrl
      ...
      <tr>
        <td style="padding:8px 32px 28px;">
          <a href="${opts.ctaUrl}" target="_blank" rel="noopener"
        ...
        <table role="presentation" width="560" cellspacing="0" cellpadding="0"
          ...
          <tr>
            <td style="background:${BRAND_RED};padding:18px 32px;">
              <span style="font-family:Inter,Segoe UI,Arial,sans-serif;font-size:18px;font-weight:700;color:#ffffff;">Gablive</span>
            </td>
          </tr>
          <tr>
          ...
          <tr>
            <td style="padding:0 32px 24px;border-top:1px solid ${BORDER};">
              <p style="margin:16px 0 0;font-family:Inter,Segoe UI,Arial,sans-serif;font-size:12px;line-height:1.5;color:${MUTED};">
                ${opts.footerNote || "Você recebeu este e-mail porque se inscreveu em um webinário na Gablive."}
              </p>
            </td>
          </tr>
        </table>
...
export function getDefaultEmailBodyHtml(type: string): string {
  switch (type) {
    case "confirmation":
      return layout({
...
export function buildResendEmailPayload(input: {
  ...
  const fallbackSubject = DEFAULT_EMAIL_SUBJECTS[input.type] || "Gablive"
  const rawSubject =
    input.subject && String(input.subject).trim()
      ? input.subject
      : fallbackSubject
  const rawHtml =
    input.bodyHtml && String(input.bodyHtml).trim()
      ? input.bodyHtml
      : getDefaultEmailBodyHtml(input.type)

  return {
    subject: applyEmailPlaceholders(rawSubject, input.vars),
    html: applyEmailPlaceholders(rawHtml, input.vars),
  }
}
